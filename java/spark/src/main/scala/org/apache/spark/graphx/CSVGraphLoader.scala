 ...
 */
object CSVGraphLoader extends Logging {
    ...
    : Graph[Int, Double] =
  {
    ...
    val edges = lines.mapPartitionsWithIndex { (pid, iter) =>
      val builder = new EdgePartitionBuilder[Double, Int]
      iter.foreach { line =>
        if (!line.isEmpty && line(0) != '#') {
          ...
          val edata = if (lineArray.length < 3) {
            1.0d
          } else {
            lineArray(2).toDouble
          }
          if (canonicalOrientation && srcId > dstId) {
            builder.add(dstId, srcId, edata)
          } else {
            builder.add(srcId, dstId, edata)
          }
        }
      }
