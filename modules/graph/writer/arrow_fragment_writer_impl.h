...
boost::leaf::result<void> ArrowFragmentWriter<FRAG_T>::WriteVertices() {
  LOG(INFO) << "Frag-" << frag_->fid() << " write vertices, vertex info num="
            << graph_info_->GetVertexInfos().size();
  for (auto& item : graph_info_->GetVertexInfos()) {
    std::string label = item.first;
    LOG(INFO) << "Frag-" << frag_->fid() << " write vertices of label: " << label;
...
boost::leaf::result<void> ArrowFragmentWriter<FRAG_T>::WriteVertex(
  ...
  if (label == "person") {
    LOG(INFO) << "label: " << label << " chunk_index_begin: " << chunk_index_begin << " fid" << frag_->fid();
  }
  ...
  if (frag_->fid() != frag_->fnum() - 1 && num_rows % vertex_info.GetChunkSize() != 0) {
    vertex_table = AppendNullsToArrowTable(vertex_table,
                                           vertex_info.GetChunkSize() -
                                           num_rows % vertex_info.GetChunkSize());
    LOG(INFO) << "label: " << label << " origin num_rows: " << num_rows
              << ", after append nulls: " << vertex_table->num_rows() ;
  }
