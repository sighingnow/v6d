...
func (c *ClientBase) doReadReply(v common.Reply) error {
	...
	if err := json.Unmarshal(messageIn, v); err != nil {
		return err
	}
	return v.Check()
}
...
func (c *ClientBase) CreateMetaData(metadata *ObjectMeta) (id types.ObjectID, err error) {
	if id, signature, instanceId, err := c.CreateData(metadata.MetaData()); err != nil {
		return id, err
	}
...
func (c *ClientBase) GetData(ids []types.ObjectID, syncRemote bool, wait bool) (objects []map[string]any, err error) {
	...
	for _, id := range ids {
		if item, ok := reply.Content[id]; ok {
...
// func (c *ClientBase) GetMetaData(id common.ObjectID, meta *ds.ObjectMeta, syncRemote bool) {
// 	if !c.connected {
// 		return
// 	}
