...
DEFINE_bool(etcd_txn_nofail, true, "Whether to retry when txn fails");
DEFINE_bool(print_etcd_traffic, false, "Log etcd traffics using glog");
// share memory
...
json EtcdSpecResolver::resolve() const {
  ...
  spec["etcd_txn_nofail"] = FLAGS_etcd_txn_nofail;
  spec["print_etcd_traffic"] = FLAGS_print_etcd_traffic;
  return spec;
}
